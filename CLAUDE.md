...
- **人気ワインランキング（2026-07-04実装）**: 方針=「食べログ的な通しの人気ランキングを軸に、年間ベストを表彰枠として発表」。backend `GET /api/top-wines?limit=`＝computeWineScoreと同一式（店舗★＋SP創業者評価＋外部評価の信頼度加重ベイズ平均）の**一括計算版**（5クエリ・10分メモリキャッシュ）。コールドスタート=評価が無いうちはSP主導→★が貯まるほどコミュニティ主導へ自然移行。フロントは prototype.html ランキングタブを4構成に改修（🏆総合人気=/api/top-wines・💰コスパ=既存osu_gbm順・🏪お店=/api/top-places・👑年間ベスト=/api/best-wines&min=2。評価不足の年は「集計中」を正直表示）。**架空ユーザーランキング（田中ソムリエール等のモック）は全廃**。
- **お店ランキング（2026-07-04実装・オーナー要望「いいワインを出す店が上がる→みんな行く」）**: backend `GET /api/top-places?limit=`＝店のリスト中マスター一致ワインのSP→★換算の**ベイズ平均（C=5,m=3.0）**＝1本の銘酒より「良いワインを何本も揃えた店」が上位。同一銘柄はユニーク化（重複登録で盛れない）。対象=①公式店（restaurant_settings.is_public×restaurant_wines.available）②街で発見（places×venue_list_items=スカウターのクラウド収集）。①②の名寄せはv1では未実装（共通キーなし）。フロントはカードタップで公式店→`?r=`のワインリスト・発見店→Googleマップ検索。10分キャッシュ。
